...
namespace Game.Lobby
{
    ...
    public class LobyManager : MonoBehaviour
    {
        ...
        public AudioSource uiSfxSource;     // ?黎??筌??????嫄????????뀀땽 AudioSource ?????ル뭸??
        public AudioClip buttonClickClip;   // ?嶺?????????????????????ル뭸??
        ...
        void LoadSelectedWorld()
        {
            ...
            {
                Debug.LogWarning("Selected world is missing or world metadata could not be found.");
                RefreshWorldList();
                return;
            }
