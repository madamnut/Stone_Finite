...
// - FIX: toolActions["Coke"].results 파싱을 List<object>/JArray + 원소 Dictionary/JObject 모두 지원
...
public class CokeOven : Multiblock
{
    ...
    public override void Tick()
    {
        ...
        // "진행도=예약" 규칙: 출력 막힘이면 작업 자체를 시작하지 않음
        bool hasWork = canCoke && !outBlocked;

        // (1) 연료가 꺼져있으면: pending 부산물 푸시 시도 → hasWork일 때만 점화 시도
        if (_fuelTicksLeft <= 0)
        {
        ...
        if (wasBurning && hasWork)
        {
            ...
            // 완료 커밋: 출력 2개 모두 가능할 때만 + 그때만 입력 소모
            if (_cokeTicksNeed > 0 && _cokeTicksDone >= _cokeTicksNeed)
            {
    ...
    )
    {
        ...
        // ✅ results를 List<object> 로 통일 (List<object> / JArray 모두 지원)
        ...
        else if (rsObj is JArray jarr)
        {
            ...
            for (int i = 0; i < jarr.Count; i++)
                list.Add(jarr[i]); // JToken 그대로 (대개 JObject)
        }
        else
        {
            return false;
