// MultiblockLibrary.cs (?ÑÏ≤¥ ÍµêÏ≤¥Î≥? - JSON row0(Îß??? -> ?¥Î? y=0(Îß??ÑÎûò)Î°??åÏã± ??Y ?§ÏßëÍ∏?
using System.Collections.Generic;
using Newtonsoft.Json.Linq;
using UnityEngine;

/// <summary>
/// Î©Ä?∞Î∏î???®ÌÑ¥/Í≤∞Í≥º ?ïÏùò + ?¨Î£å(?Ä ?¥Î¶Ñ) ??Î©Ä?∞Î∏î???ÑÎ≥¥ ??°∞???ÑÎã¥.
/// (?ïÌôï Îß§Ïπ≠: pattern??null/ÎπàÎ¨∏?êÏó¥/?ÑÎùΩ ?àÏö©?òÏ? ?äÏùå)
///
/// JSON ?¨Îß∑(??:
/// {
///   "Clay Kiln": [
...
public class MultiblockLibrary : MonoBehaviour
{
    ...
    [Tooltip("Î©Ä?∞Î∏î???ïÏùò JSON (?? Clay Kiln ?®ÌÑ¥/Í≤∞Í≥º ??")]
    ...
    void Awake()
    {
        ...
        {
            Debug.LogError("[MultiblockLibrary] multiblockJson ??ÎπÑÏñ¥?àÏäµ?àÎã§.");
            return;
        }
    ...
    static bool LoadFromJson(string json)
    {
        ...
        foreach (var prop in root.Properties())
        {
            ...
            for (int i = 0; i < arr.Count; i++)
            {
                ...
                // ??JSON?Ä row0=Îß??? ?¥Î? Î∞∞Ïó¥?Ä y=0=Îß??ÑÎûòÎ°??∞Í∏∞ ?ÑÌï¥ YÎ•??§Ïßë?¥ÏÑú ?Ä?•Ìïú??
                ...
                // ingredient index: pattern ?¥Î????Ä ?¥Î¶Ñ ?ÑÎ? ?¨Î£åÎ°??±Î°ù
                for (int y = 0; y < height; y++)
                {
                    for (int x = 0; x < width; x++)
                    {
