...
namespace Game.Data
{
    public partial class RecipeLibrary
    {
            ...
            out JObject matchedRecipe)
        {
            ...
            // ?곗꽑?쒖쐞: ?꾩옱 ?뚯씠釉??ш린 ?덉떆???????묒? 寃⑹옄 ?덉떆???щ씪?대뵫)
            if (n == 16)
            {
            ...
            out JObject matchedRecipe)
        {
            ...
            for (int rix = 0; rix < recipeSet.Count; rix++)
            {
                ...
                var oaRoot = r["outputActions"] as JArray; // output蹂?action list (null 媛??
    
                int[] assign = null;
    
                if (isShapeless)
                {
                    // shapeless: inputs??"議댁옱?섎뒗 留뚰겮留? ??filledCount == inputs.Count
                ...
                {
                    // shaped: inputs??寃⑹옄 ?ш린(2/4/9/16) 洹몃?濡? null濡?鍮덉뭏 ?쒗쁽 媛??
        ...
        bool TryMatchShapedWithSlidingAndTransforms(JArray recipeInputs, List<ItemData> slots, out int[] assign)
        {
            ...
            for (int i = 0; i < rw * rh; i++)
            {
                ...
                baseGrid.cells[i] = recipeInputs[i] as JObject; // null 媛??
            }
    
            // ??2-slot shaped??"?좏삎 ?쒖옉踰? 蹂댄샇: transforms(?뚯쟾/?移? 湲덉?
        ...
        {
            // null/鍮?null 諛??듭떖 ?ㅻ쭔?쇰줈 媛꾨떒 fingerprint
            // (異⑸룎 媛?μ꽦? ??퀬, 異⑸룎 ??以묐났 variant留??섏뼱??
