...
namespace Game.UI
{
    public partial class CokeOvenModule : MonoBehaviour
    {
        ...
        public Image progressGauge; // Coke ??????癲ル슣???몄춿?????ャ뀕??
        ...
        void Update()
        {
            ...
            // 2) ????곸죷 ??????怨뚮뼚????源끹걬癲??????좊읈? ?壤굿??苡?????琉?????袁ⓓ??怨뚮뼚??? -> Oven???袁⑸즵???
            if (InputsChanged())
            {
                PushInputsToOven();
                CaptureInputSnapshots();
